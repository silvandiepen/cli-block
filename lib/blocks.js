...
const { border, defaultSettings } = require("./border");
...
const CONTENT_WIDTH = FRAME_WIDTH - (FRAME_WIDTH / 10) * 2;
...
// The basic line.
const BLOCK_LINE = (msg = null, settings = { prepend: "" }) => {
	if (settings) settings = Object.assign(defaultSettings, settings);

	if (msg == null) {
		LOGG(
			`${border("side", settings)}${spaces(FRAME_WIDTH)}${border(
				"side",
				settings
			)}`
		);
	...
	msg.forEach((txt, i) => {
		if (i == 0) txt = `${settings.prepend ? settings.prepend + " " : ""}${txt}`;
		else
			txt = `${
				settings.prepend ? spaces(stringWidth(settings.prepend)) + " " : ""
			}${txt}`;

		LOGG(
			`${border("side", settings)}${spaces(FRAME_WIDTH / 10)}${txt}${spaces(
...
const BLOCK_ROW_LINE = (arr) => {
	if (settings) settings = Object.assign(defaultSettings, settings);

	let str = "";
	arr = arr.map((item) => toStringValue(item));
	let COLUMN_WIDTH = ((FRAME_WIDTH - CONTENT_WIDTH) * 2) / arr.length;
...
const BLOCK_START = (txt = null, settings = null) => {
	...
	if (txt)
		LOGG(
			`${border("topStart", settings)}${repeat(
...
const BLOCK_MID = (txt = null, settings = null) => {
	...
	if (txt)
		LOGG(
			`${border("midStart", settings)}${repeat(
				Math.floor(FRAME_WIDTH / 3),
...
// Closing Block
const BLOCK_END = (txt, settings) => {
	if (settings) settings = Object.assign(defaultSettings, settings);

	BLOCK_LINE(null, settings);
	LOGG(
		`${border("bottomStart", settings)}${repeat(
			FRAME_WIDTH,
			`${border("line", settings)}`
...
const BLOCK_SETTINGS = async (obj, settings) => {
	if (settings) settings = Object.assign(defaultSettings, settings);
...
const BLOCK_WARNINGS = (warning, settings) => {
	if (settings) settings = Object.assign(defaultSettings, settings);
...
const BLOCK_ERRORS = (error, settings) => {
	if (settings) settings = Object.assign(defaultSettings, settings);
